...
class HoloKVStoreGetValue(Service):
    ...
    def handle(self):
        ...
        if not self.checkKey(data, 'kv_store'):
            response = {"error": "request must contain a kv store namespace identifier"}
            self.response.status_code = httplib.BAD_REQUEST
            self.response.payload = dumps(response)
            return
        if not data.kv_store:
            response = {"error": "kv_store value must not be empty"}
            self.response.status_code = httplib.BAD_REQUEST
            self.response.payload = dumps(response)
            return
        ...
        # get the id for the kv_store
        namespace = data.kv_store
        if namespace == "dns2hash":
            namespace = self.kvdb_get( 'cloudflare', 'kvstore_dns2hash' )
        if namespace == "hash2tranche":
        ...
        conn = self.outgoing.plain_http['cloudflare-kvstore-get-value'].conn
        ...
        if not response:
            ...
            """
        # else
        self.response.payload = dumps({'value':value})
        return
...
outconn:cloudflare-kvstore-get-value:
    https://api.cloudflare.com
    /client/v4/accounts/{account}/storage/kv/namespaces/{namespace}/keys/{kv_key}
    Cloudflare API
