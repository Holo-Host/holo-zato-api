...
class HoloDnsCustomerDelete(Service):
    ...
    def handle(self):
        ...
        # exit if no domain incoming
        if not self.checkKey(data, 'domain'):
            response = {"error": "request must contain a domain"}
        ...
        # params
        domain = data.domain + ".holohost.net"
        dtype = "CNAME"
